...
class WBCQP:
    ...
    def _build_template(self):
        ...
        # polish=False: the polish refactorization is the classic source of repeated
        # "ERROR in osqp_update_data_mat: new KKT matrix is not quasidefinite" spam on
        # degenerate/ill-conditioned WBC problems, and a failed polish/update can make
        # solve() return x=None -> zeros -> the controller "freezes". We don't need the
        # extra accuracy. sigma raised 1e-6 -> 1e-4 to keep P+sigma*I comfortably PD so the
        # KKT stays quasidefinite when P/A are updated each tick.
        self._prob.setup(P=self.P, q=self.q, A=self.A, l=self.l, u=self.u,
                         verbose=False, polish=False, sigma=1e-4)
    ...
    def update_and_solve(
        ...
        *,
        ...
        if in_stance:
            ...
            lb[0:3] = -np.inf
            ub[0:3] = np.inf
            # Paper-style HFA stance allocation: fz is prescribed by vertical
            # energy/height control, while the QP only chooses fxy for attitude.
            if fix_fz is not None:
                fz_cmd = float(max(0.0, float(fix_fz)))
                lb[2] = fz_cmd
                ub[2] = fz_cmd
        ...
        # Robust fallback: if the solver fails (x=None / NaN), reuse the last valid
        # solution instead of zeroing every output (which freezes the controller).
        if res.x is not None and np.all(np.isfinite(res.x)):
            x = np.asarray(res.x, dtype=float)
            self._x_prev = x.copy()
        elif getattr(self, "_x_prev", None) is not None:
            x = self._x_prev.copy()
        else:
