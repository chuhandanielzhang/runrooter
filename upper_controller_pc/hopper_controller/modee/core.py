...
class ModeEConfig:
    ...
    # kinematic vz estimate).  2026-08-02: 15 -> 8 ms.  The NRC pump is
    # lag-sensitive: it reads a stale vz, thinks r < r* while the true
    # velocity already passed v_to, and keeps injecting (log 073212:
    # r/r* read 0.82 at push end while vz was 1.32 vs the 1.166 target
    # -> apex 12-18 cm on a 7 cm setpoint).
    ...
    # 2026-08-02: 400 -> 250.  The paper's within-stance convergence
    # assumes the TRUE vz; through a lagged estimate the pump over-
    # injects in proportion to kR*lag (sim: 30 ms of effective lag at
    # kR=400 lands the 7 cm setpoint at 11 cm -- exactly the logged
    # overshoot).  250 with the 8 ms vz LPF sims at 6.6 cm and is 2.6x
    # less sensitive to whatever pipeline lag remains.
    nrc_kR: float = 250.0                # norm-regulation gain [1/(m*s)]
