...
# NOTE: disabled (0) -> use the RAW kinematics/Jacobian output, no coordinate rotation.
KIN_YAW_OFFSET_DEG = 0.0

# ===== Motor index convention alignment (physical motors -> analytic model) =====
# NOTE: disabled (identity) -> use the RAW motor order, no permutation.
PERM_MODEL_FROM_PHYS = np.array([0, 1, 2], dtype=int)   # identity (no reorder)
PERM_PHYS_FROM_MODEL = np.array([0, 1, 2], dtype=int)   # identity (no reorder)
