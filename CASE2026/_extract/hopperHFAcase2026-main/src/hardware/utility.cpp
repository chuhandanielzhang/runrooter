...
canChannel::canChannel() {
    ...
    // 🔧 初始化 motor_state 防止读取未初始化数据
    for (int i = 0; i < 255; i++) {
        motor_state[i][0] = 0.0f;
        motor_state[i][1] = 0.0f;
        motor_state[i][2] = 0.0f;
    }
}
...
void canChannel::comm_init(uint8_t port_num, uint8_t device_num) {
    ...
    //Initiallize - CAN ID 从 1 开始 (1,2,3)
    for(uint8_t i=0; i<numOfDevice; i++){ 
        send_ExitMotorMode(i+1);  // CAN ID = 1, 2, 3
