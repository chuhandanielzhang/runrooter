    ...
    def _log_step(
        ...
        *,
        ...
        try:
            ...
            hop_peak_z_m = float(info.get("hop_peak_z_m", float("nan")))
    ...
    def run_controller(self) -> None:
        ...
        try:
            while self.running:
                ...
                # Print gamepad + desired velocity only (compact, ~5 Hz)
                self._dbg_step_ctr = getattr(self, "_dbg_step_ctr", 0) + 1
                if self._dbg_step_ctr % 100 == 0:
                    gp = self.robot_state.get("gamepad")
                    sx = sy = 0.0
                    try:
                        sx = float(gp.rightStickAnalog[0]) if gp else 0.0
                        sy = float(gp.rightStickAnalog[1]) if gp else 0.0
                    except Exception:
                        pass
                    print(f"stick=[{sx:+.2f},{sy:+.2f}]  v_des=[{desired_v_xy[0]:+.3f},{desired_v_xy[1]:+.3f}] m/s")
